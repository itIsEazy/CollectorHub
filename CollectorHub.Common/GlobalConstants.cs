namespace CollectorHub.Common
{
    public static class GlobalConstants
    {
        //----------------------------Common-------------------------------//
        public const string SystemName = "CollectorHub";

        public const string AdministratorRoleName = "Administrator";

        //----------------------------Sorting-------------------------------//
        public const string SortingCheapestName = "Cheapest";
        public const string SortingExpensivestName = "Expensivest";
        public const string SortingOldestName = "Oldest";
        public const string SortingNewestName = "Newest";
        public const string SortingLessViewedName = "Less Viewed";
        public const string SortingMostViewedName = "Most Viewed";

        //----------------------------Collections-------------------------------//

        //----------------------------CollectionTypes-------------------------------//
        public const string LegoCollectionTypeName = "Lego";
        public const string HotWheelsCollectionTypeName = "Hot Wheels";

        public const string LegoStarWarsTypeName = "Star Wars";

        //----------------------------Categories-------------------------------//
        public const string LegoCategoryName = "Lego";
        public const string HotWheelsCategoryName = "Hot Wheels";

        //----------------------------HotWheels-------------------------------//
        public const string FastAndFuriousTypeName = "Fast and Furious";
        public const string FastAndFuriousPremiumTypeName = "Fast and Furious Premium";
    }
}
